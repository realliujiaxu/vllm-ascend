                ...
                if (
                    ...
                    if isinstance(
                        ...
                        assert current_kv_cache_spec.page_size_padded is None, (
                            "MiniMax M3 Dense FP8 KV cache does not support "
                            "padded pages."
                        )
                        assert (
                        ...
                        k_tensor = self._allocate_int8_cache_tensor(
                            current_kv_cache_spec.k_page_size_bytes
                            * num_blocks,
                            alignment,
                        )
                        v_tensor = self._allocate_int8_cache_tensor(
                            current_kv_cache_spec.v_page_size_bytes
                            * num_blocks,
                            alignment,
                        )
                        k_scale_tensor = self._allocate_int8_cache_tensor(
                            current_kv_cache_spec.k_scale_page_size_bytes
                            * num_blocks,
                            alignment,
                        )
                        for layer_name_inner in kv_cache_tensor.shared_by:
            ...
            for layer_name in group.layer_names:
                ...
                if self.use_compress and isinstance(current_kv_cache_spec,
                    ...
                    if isinstance(
                        ...
                        k_cache = raw_k_tensor.view(
                            current_kv_cache_spec.dtype
                        ).view(k_shape)
                        v_cache = raw_v_tensor.view(
                            current_kv_cache_spec.dtype
                        ).view(v_shape)
