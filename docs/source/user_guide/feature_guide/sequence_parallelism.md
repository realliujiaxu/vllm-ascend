...
- `"enable_sp"`: This is the switch for SP. Since SP relies on graph mode, it is not supported in eager mode. SP is compatible with both `enable_npugraph_ex` enabled and disabled; you can use SP together with npugraph_ex for better graph optimization.
